...
class ProcessData(Proc):
    ...
    def pids(self):
        """Get a list of PID's"""
        return [int(e) for e in self.listdir() if e.isdigit() and not self._iskernel(e)]
...
def pidmaps(pid):
    ...
    if options.mapfilter:
        ...
        for m in maps:
            if not list(
                    filters(options.mapfilter, m, lambda x: maps[x]['name'])):
                f[m] = maps[m]
        return f
    return maps
...
def maptotals(pids):
    totals = {}
    for pid in pids:
        if (filters(options.processfilter, pid, proc.pidcmd)
